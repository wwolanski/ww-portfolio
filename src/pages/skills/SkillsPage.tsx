    ...
    >
      <section className="content-section skills-showcase-section">
        <div className="skills-showcase">
          <p className="topline">{content.showcase.eyebrow}</p>
          <h2 className="headline">{content.showcase.title.map((line, index) => <span key={line}>{line}{index < content.showcase.title.length - 1 && <br />}</span>)}<span className="accent-dot">.</span></h2>
          <p className="intro">{content.showcase.intro}</p>
            ...
            <div className="skills-columns">
              {content.showcase.columns.map((column, index) => (
                <article className={`skill-column ${skillColumnColors[index] ?? 'green'}`} key={column.title}>
                  ...
                  <h3>{column.title}</h3>
                  <ul className="skill-list">{column.skills.map((skill) => <li key={skill}>{skill}</li>)}</ul>
                </article>
              ))}
            </div>
            <div className="soft-skills">
              <h3 className="soft-title">{content.showcase.softTitle}</h3>
